 ...
 
{
    ...
    *length = 0;
    ...
    //create a char array with a size of BYTES.
    BYTE buffer[BYTES];
    size_t bytes;
    
    //read the file 
   while((bytes=fread(buffer, sizeof(BYTE), 1, file))!=0)
   {
       if(file == NULL){
           return false;
       } 
        ...
        *content = realloc(*content, *length + bytes + 1);
        ...
        /*Copies the values of bytes pointed to by buffer directly 
        to the memory block to pointed by destination.*/
...
{   
    //create a char array with a size of "GET ".
    char method[4];

    //create a char array with a byte size of the request target.
    char req_target[strlen(line)-strlen("HTTP/1.1")-strlen("GET")-3]; 
 
    //iterate over the request line to store the string "GET".
    for (int index = 0; index < strlen(line); index++)
    {
        method[index]= line[index];
        if (index  == 2)
        {
        break;
        }
    ...
    //compare the string with "GET". If not the same, return error.
    ...
    //if HTTP/1.1 is not found in the request line, return error.
    ...
    //if the first slash is not found, return error.
    if (line[4]!= '/')
    {
        error(501);
        return false;
    ...
    //count the number of spaces in the request line.
    int spaceCount = 0;
    ...
    if (spaceCount == 2){
        ...
        //store the string after the first slash.
        char* start = strchr(line, '/');
        
        //store the string between the first slash but before the second space.
        char* req_target_token  = strtok(start, " ");
        
        //copy the string into the array
        strcpy(req_target, req_target_token);
        
        //search for the "".
        char* quote = strchr(req_target, '"');
        
        //search for the ?.
        char* question_mark = strchr(req_target, '?');
        
        //if "" is present in the req_target, return error.
        if(quote)
        {
            error(400);
            return false;
            ...
            if (question_mark)
            {
                //store the string from the first slash but before '?'.
                char* abs_path_token = strtok(req_target, "?");
                strcpy(abs_path, abs_path_token);
                
                int j = 0;
                while (j< strlen(line) - 1)
                {
                    //if the space is right after '?', query is 0.
                    ...
                    //if no space is right after '?', store the query string after '?'.
                ...
                //store req_target in abs_path if '?' is not present in req_target.
                query = "";
    ...
    //return error if the space count is not equal to 2.
    else{
        error(400);
        return false;
    }
